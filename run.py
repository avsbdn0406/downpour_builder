    ...
    if c_idx == 0:
        ...
        if 'data_01' in st.session_state and 'data_02' in st.session_state and 'data_03' in st.session_state:
            ...
            if 'levels' not in st.session_state:
                ...
                st.session_state.library, st.session_state.levels = save_and_processing(st.session_state.save_path,st.session_state.grid, st.session_state.df, st.session_state.category)
        ...
        if 'dataset' in st.session_state and 'data_01' in st.session_state and 'data_02' in st.session_state and 'data_03' in st.session_state:
            ...
            if 'levels' not in st.session_state:
                ...
                st.session_state.tot_child ,st.session_state.sido_df = make_tot_sido(st.session_state.data_01,
                                                                                     st.session_state.data_02,
                                                                                     st.session_state.data_03)
                ...
                st.session_state.results,st.session_state.time_nm, st.session_state.df_list = run_people(st.session_state.dataset, st.session_state.base_grid,
                           st.session_state.seoul_grid, st.session_state.tot_child,
                           st.session_state.sido_df)
            ...
            if 'images' in st.session_state and 'libraries' in st.session_state and 'levels' in st.session_state:
                for f_name, level, library, img in zip(st.session_state.save_files, st.session_state.levels,
                    ...
                    show_button(st.session_state.save_path, level, img, library, st.session_state.category, f_name)

    elif c_idx == 2:
        dataset= []
        st.session_state.category = '도로'

        st.write('도로 데이터 X 전국격자지도')

        uploaded_file = st.file_uploader('Please choose a file.', accept_multiple_files=False, type='csv', key='0')

        if uploaded_file is not None and 'road_overlay' not in st.session_state:
            placeholder = st.empty()
            placeholder.success(uploaded_file.name + ' 데이터 읽는 중...')
            ...
            st.session_state.road_overlay = df
            placeholder.success('도로 x 전국격자지도 데이터 읽기 완료')
        ...
        if 'road_overlay' in st.session_state:
            ...
            if 'levels' not in st.session_state:
                ...
                st.session_state.road_overlay['length_union'] = st.session_state.road_overlay.length / 1000

                st.session_state.df = make_traffic(st.session_state.road_overlay)
        ...
        if tmp is not None and len(tmp)!=0 and 'buld' not in st.session_state:
            ...
            for idx, uploaded_file in enumerate(tmp):
                ...
                if uploaded_file.name.split('.')[-1]=='csv':
                    ...
                    df = gpd.GeoDataFrame(df, crs='epsg:5179')
                    dataset.append(df)
            st.session_state.buld = dataset #pd.concat(dataset, ignore_index=True)
        ...
        if tmp is not None and len(tmp) !=0 and 'emd' not in st.session_state:
            ...
            st.session_state.emd = dataset #pd.concat(dataset, ignore_index=True)
        ...
        if 'buld' in st.session_state:
            ...
            if 'sub_cidx' not in st.session_state or sub_category != st.session_state.sub_cidx:
                st.session_state.sub_cidx = sub_category
                st.session_state.save_path = './results/'

                st.session_state.libraries, st.session_state.levels, st.session_state.images = run_facil(st.session_state.sub_cidx, st.session_state.buld,
                                                st.session_state.list_name, st.session_state.grid)


            if 'images' in st.session_state and 'libraries' in st.session_state and 'levels' in st.session_state:
                for st.session_state.library, st.session_state.level, st.session_state.img, c in zip(st.session_state.libraries, st.session_state.levels, st.session_state.images, st.session_state.c_list):
